document.addEventListener("DOMContentLoaded", function () {
        ...
        if (isMultipleChoice) {
            if (!optionsContainer) {
                ...
                optionsHeader.innerHTML = `
                    <label class="form-label fw-bold mb-3">Các lựa chọn <span class="text-danger">*</span></label>
                ...
                addOptionBtn.className = "btn btn-outline-primary btn-sm mt-2 add-option";
    ...
    questionsContainer.addEventListener("click", function (e) {
        if (e.target.classList.contains("remove-option") || e.target.closest(".remove-option")) {
            ...
            if (optionItems.length > 2) {
                ...
                setTimeout(() => {
                    const wasChecked = optionItem.querySelector('.correct-option').checked;
                    optionItem.remove();
    ...
    function updateOptionIndices(optionsContainer) {
        ...
        optionItems.forEach((item, index) => {
            ...
            inputs.forEach(input => {
                if (input.name) {
                    input.name = input.name.replace(/Questions\[\d+\]\.Options\[\d+\]/, `Questions[${questionIndex}].Options[${index}]`);
                }
                if (input.type === "radio") {
    ...
    // Lấy tất cả các trường câu hỏi và điểm tối đa
    function getQuestionFields() {
        return Array.from(document.querySelectorAll('.question-item')).map(qb => {
            return {
                type: qb.querySelector('select[name$=".QuestionType"]')?.value || '',
                content: qb.querySelector('textarea[name$=".QuestionText"]')?.value || '',
                maxScore: qb.querySelector('input[name$=".MaxScore"]')?.value || ''
            };
        });
    }
    ...
    if (titleInput && descInput && saveBtn) {
        ...
        function isChanged() {
            ...
            if (titleNow !== origTitle || descNow !== origDesc) return true;
            // So sánh từng câu hỏi
            const nowQuestions = getQuestionFields();
            if (nowQuestions.length !== original.questions.length) return true;
            for (let i = 0; i < nowQuestions.length; i++) {
                if (nowQuestions[i].type !== original.questions[i].type) return true;
                if (nowQuestions[i].content.trim() !== original.questions[i].content.trim()) return true;
                if (nowQuestions[i].maxScore.trim() !== original.questions[i].maxScore.trim()) return true;
            }
            return false;
        }
        ...
        titleInput.addEventListener('input', checkChanged);
